...
feature "visitor sorts projects" do
  let!(:project1) { create(:commitwith, open_issues: 1, size: 1000) }
  ...
  scenario "visitor sorts projects by number of open issues", js: true do
    ...
    expect(page.text).to match(/rails.*glide.*commitwith/)

    click_link "sort-open_issues"

    expect(page.text).to match(/commitwith.*glide.*rails/)
  end
